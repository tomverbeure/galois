...
./build/tb_gf_arith_mastrovito: ./verif/tb_gf_arith.cc ./build/gf_poly_mult_mastrovito.h 
	$(CPLUSPLUS) -O2 -DMASTROVITO -DDESIGN_TOP=p_gf$(GF)__poly__mult__mastrovito -DGF=$(GF) -std=c++11 -I`yosys-config --datdir`/include/backends/cxxrtl/runtime -I./build $< -o $@

./build/tb_gf_arith: ./verif/tb_gf_arith.cc ./build/gf_poly_mult.h
	$(CPLUSPLUS) -O2 -DDESIGN_TOP=p_gf$(GF)__poly__mult -DGF=$(GF) -std=c++11 -I`yosys-config --datdir`/include/backends/cxxrtl/runtime -I./build $< -o $@
